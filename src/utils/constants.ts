...
// Orchestration webhook (intent routing / workflow). Still used by the
// in-device build for local-intent-assisted orchestration. NOTE: this is a
// lab endpoint and must eventually be replaced for a fully standalone,
// App-Store-publishable ic4u build (see README → "Remaining backend deps").
...
// The following three backend pipelines were dropped because they are not
// generalisable / App-Store-publishable. ic4u runs reaching + navigation fully
// on-device via ARKit (Spatial Target reaching + ARKit route navigation):
//   • SMART_GUIDANCE_URL  — Melody's tracker-driven reaching pipeline
//   • RTAB_GUIDANCE_URL   — Kasra's RTAB-Map indoor navigation pipeline
//   • DETECTION_URL       — Qwen VLM bbox detection / progressive re-detection
//   • ACQUISITION_URL     — Qwen-path acquisition validation webhook
// Do not re-introduce these here; on-device replacements live in the native
// ARKit ReachingModule / SemanticRouteNavigator.
