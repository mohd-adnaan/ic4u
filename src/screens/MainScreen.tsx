...
const MainScreen: React.FC = () => {
        ...
        if (isScreenReaderOn && hasPermissions) {
          setTimeout(() => {
            AccessibilityInfo.announceForAccessibility(
              'ic4u activated. Use the voice button to start recording. Double tap the button to take a photo immediately.'
            );
          }, 1000);
        }
      } catch (error) {
  ...
  const initializePermissions = async () => {
    try {
      ...
      if (!granted) {
        ...
        const errorMessage = 'ic4u needs microphone and camera access to function properly. Please grant these permissions in your device settings.';
        ...
        // Announce success
        AccessibilityInfo.announceForAccessibility(
          'Permissions granted. ic4u is ready to use.'
        );
      }
    } catch (error) {
  ...
  const handlePress = async () => {
    ...
    if (!hasPermissions) {
      const message = 'Please grant microphone and camera permissions to use ic4u.';
    ...
    if (isSpeaking) {
      ...
      AccessibilityInfo.announceForAccessibility(
        'Speech stopped. ic4u is ready. Tap to speak.'
      );
      return;
    }
    ...
    if (isProcessing) {
      ...
      // Inform user
      AccessibilityInfo.announceForAccessibility(
        'ic4u is processing. Please wait, or double tap to cancel.'
      );
      return;
    }
  ...
  const handleDoubleTap = async () => {
    ...
    if (isSpeaking) {
      ...
      // Announce ready
      AccessibilityInfo.announceForAccessibility(
        'Stopped. ic4u is ready. Tap to speak.'
      );
      return;
    }
  ...
  const processVoiceCommand = async (transcript: string) => {
    ...
    try {
      ...
      AccessibilityInfo.announceForAccessibility(
        'Response complete. ic4u is ready. Tap to speak.'
      );
