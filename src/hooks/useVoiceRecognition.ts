    ...
    if (typeof errorCode === 'string') {
      ...
      if (errorStr.includes('permission')) {
        ...
        const guidance = Platform.OS === 'ios'
          ? 'To enable: Settings → Privacy → Microphone → ic4u'
          : 'To enable: Settings → Apps → ic4u → Permissions → Microphone';
      ...
      try {
        ...
        if (error.message) {
          ...
          if (errorMsg.includes('permission')) {
            const guidance = Platform.OS === 'ios'
              ? 'Settings → Privacy → Microphone → ic4u'
              : 'Settings → Apps → ic4u → Permissions → Microphone';
